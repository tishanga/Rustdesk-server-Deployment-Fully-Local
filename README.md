# RustDesk LAN Server – Custom Installer

This repository provides a **LAN‑first RustDesk server and client installation workflow**.

It solves a common issue with the upstream RustDesk `install.sh`:

> The official script embeds the **public WAN IP** into the configuration key, which breaks **LAN‑only or internal deployments** and makes mass client rollout impractical.
...
* Hosts **pre‑configured Windows & Linux client installers** via an internal HTTP server
...
.
├── server/
│   └── install.sh              # LAN‑only RustDesk server installer
├── clients/
...
## 🚀 Server Installation (LAN Only)

### ⚠️ Important – LAN IP Configuration
...
✔ No manual client reconfiguration is required
...
* Ensure ports **21115–21119/TCP** are open
...
### 3️⃣ Configure LAN IP (Required)

You **must edit the installer before running it**.

> ⚠️ Root or sudo privileges are required to edit the file.
...
✔ This replaces the default WAN IP auto-detection
✔ Ensures the generated configuration key works for LAN clients
...
## 🪟 Windows Client (One‑Click BAT Installer)
...
✔ No manual RustDesk configuration

---

## 🐧 Linux Client (Automated Script)
...
RustDesk’s official installer is WAN‑first by design.

In **enterprise / SOC / lab / offline networks**, this causes:

* Broken auto‑config keys
